...
import { SystemProgram, SYSVAR_SLOT_HASHES_PUBKEY, PublicKey } from '@solana/web3.js';
...
export const mintOneTokenNew = async (
  ...
  if(burns !== undefined) {
    burns.map((burn) => {
      instructions.push(Token.createBurnInstruction(
        TOKEN_PROGRAM_ID,
        new PublicKey(burn.tokenMintAddress),
        new PublicKey(burn.tokenAccountAddress),
        payer,
        [],
        1
      ));
